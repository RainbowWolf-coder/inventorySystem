...
$endpoint = ($BaseUrl.TrimEnd('/') + "/api/processStockAdjustments")

Write-Host "POST $endpoint (max=$MaxToProcess)"

try {
  $headers = @{ 'x-sync-token' = $syncToken }
  $payload = @{ max = $MaxToProcess } | ConvertTo-Json
  $result = Invoke-RestMethod -Uri $endpoint -Method Post -Headers $headers -ContentType 'application/json' -Body $payload
